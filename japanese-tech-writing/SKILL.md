...
description: 日本語の技術文書・書籍原稿の文章規範。整形（一文一行、引用ブロック、脚注、コラム記法）、段落と論証の構成（パラグラフライティング）、論証の厳密さ（ツッコミどころの除去）、読み手の負荷の管理、視点と語り、演出の抑制、LLM っぽい空句の禁止、冗長の排除を定める。日本語で技術書の章、草稿、記事、解説文を書くとき、または推敲・リライトするときに使用する。英語の記事の編集は edit-article。
license: Unlicense
metadata:
  provenance: derived
  upstream: gist:k16shikano/fd287c3133457c4fd8f5601d34aa817d@5ed08e447536:SKILL.md
  upstream-license: Unlicense
  changes: "identical to the gist revision (which already carries the frontmatter) except the description, which gained the pointer to the sibling edit-article skill in #7"
---
