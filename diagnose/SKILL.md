...
**Then ask: what would have prevented this bug?** If the answer involves architectural change (no good test seam, tangled callers, hidden coupling) hand off to an architecture-improvement skill if one is available (for example `/improve-codebase-architecture`) with the specifics; otherwise write the recommendation into the PR. Make the recommendation **after** the fix is in, not before — you have more information now than when you started.
